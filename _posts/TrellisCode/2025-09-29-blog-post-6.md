...
0. make metadata.csv file
```python
def get_first_directory(path):  # 返回 路径下第一个子目录的名字
...
if __name__ == '__main__':
    ...
    if os.path.exists(os.path.join(opt.output_dir, 'metadata.csv')):
        ...
        metadata = dataset_utils.get_metadata(**opt)    # [sha256,file_identifier,aesthetic_score,captions]
    metadata.set_index('sha256', inplace=True)
    
    # merge downloaded 把处理过的信息merge起来
...
Image models: []
Latent models: []
Sparse Structure latent models: []
Statistics:
  - Number of assets: 4485
  - Number of assets downloaded: 0
  - Number of assets rendered: 0
  - Number of assets voxelized: 0
  - Number of assets with captions: 4390
  - Number of assets with image conditions: 0
```

Next
======

1. download raw data from the website
